// Based off of CodeMirror mode perl files:
...
type Tokener = (stream: StringStream, state: State) => string | null;
...
const RXstyle = 'string.special'; // 'builtin'

// NOTE: "m", "s", "y" and "tr" need to correct real modifiers for each regexp type
const RXmodifiers = /[goseximacplud]/;

// "look" is like "peek", but need for a look-ahead or look-behind if index < 0
const look = (stream: StringStream, c?: number) => stream.string.charAt(stream.pos + (c || 0));

// Return the part of the stream before the current position.
const prefix = (stream: StringStream, c?: number) => {
	if (c) {
		const x = stream.pos - c;
		return stream.string.substring(x >= 0 ? x : 0, stream.pos);
	} else {
		return stream.string.substring(0, stream.pos - 1);
	}
};

// Return the part of the current stream after the current position.
const suffix = (stream: StringStream, c?: number) => {
	const y = stream.string.length;
	return stream.string.substring(stream.pos, c && c < y ? stream.pos + c : y + 1);
};

// Skip the part of stream from the current position to the offset c.
const eatSuffix = (stream: StringStream, c: number) => {
	const x = stream.pos + c;
	let y: number;
	if (x <= 0) stream.pos = 0;
...
const tokenSOMETHING = (stream: StringStream, state: State, string: string) => {
	state.tokenize = (stream, state) => {
...
const tokenEV3 = (stream: StringStream, state: State, string: string, style: string, prevState?: State) => {
	state.tokenize = (stream, state) => {
		if (stream.match(prevState && prevState.mode == 'math' ? new RegExp(`^\\\\${string}`) : new RegExp(`^${string}`))) {
			if (!prevState) {
		...
		const newPrevState: State = { prevState: prevState ? JSON.parse(JSON.stringify(prevState)) : null, style, string };
		...
		if (prevState && prevState.mode == 'cmd') {
			...
			if (stream.match(/^[$@%]{/)) {
				// ${, @{, %{ nested blocks
				style = 'variable';
				state.tokenize = (stream, state) => tokenEV3(stream, state, '\\}', style, newPrevState);
				return style;
			}
			if (stream.match(/^\(/)) {
				// Nested ( ) blocks
			...
			if (stream.match(/^['"]/)) {
				// Quotes
				return tokenChain(stream, state, [stream.current()], 'string', null, (stream, state) =>
		...
		if (stream.match(/^\\\(/)) {
			...
			// ``...`` math object math
			style = prevState && prevState.mode != 'math' ? 'error' : 'variable-3';
			state.tokenize = (stream, state) => tokenEV3(stream, state, '``', style, newPrevState);
		} else if (stream.match(/^`/)) {
			// `...` math object math
			style = prevState && prevState.mode != 'math' ? 'error' : 'variable-3';
			state.tokenize = (stream, state) => tokenEV3(stream, state, '`([^`]|$)', style, newPrevState /*, 'tick' */);
		} else if (stream.match(/^(\$BBOLD|\${BBOLD})/)) {
			// Bold
			style += ' strong';
			state.tokenize = (stream, state) => tokenEV3(stream, state, '(\\$EBOLD|\\${EBOLD})', style, newPrevState);
			return `${PGstyle} strong`;
		} else if (stream.match(/^(\$BITALIC|\${BITALIC})/)) {
			// Italic
			style += ' emphasis';
			state.tokenize = (stream, state) => tokenEV3(stream, state, '(\\$EITALIC|\\${EITALIC})', style, newPrevState);
			return `${PGstyle} emphasis`;
...
const tokenPGML = (stream: StringStream, state: State, string: string, style?: string, prevState?: State) => {
	state.tokenize = (stream, state) => {
		...
		if (stream.match(/^\[:{1,3}/)) {
			...
			style = 'tag';
			const endstring = stream.current().substring(1) + '\\]';
			state.tokenize = (stream, state) => tokenPGML(stream, state, endstring, style, newPrevState);
		} else if (!prevState && stream.match(/^```/)) {
			// Multiline verbatim / code
			style = 'tag';
			state.tokenize = (stream, state) => tokenPGMLComment(stream, state, '```', style, newPrevState);
		} else if (stream.match(/^\[%/)) {
			// Comment
			style = 'bracket';
			state.tokenize = (stream, state) => tokenPGMLComment(stream, state, '%\\]', style, newPrevState);
			...
			// Answer blank
			if (stream.match(/^\*?\{/)) {
				state.tokenize = (stream, state) => tokenPGMLSubBlock(stream, state, '\\}', 'builtin', newPrevState);
			}
			return 'builtin';
		} else if (stream.match(/<< *$/)) {
			// Justification
...
const tokenPerl = (stream: StringStream, state: State) => {
	...
	if (stream.match(/^BEGIN_TEXT\s*$/)) {
		state.tokenize = (stream, state) => tokenEV3(stream, state, 'END_TEXT', 'meta');
		return PGkeyword;
	}
	if (stream.match(/^BEGIN_HINT\s*$/)) {
		state.tokenize = (stream, state) => tokenEV3(stream, state, 'END_HINT', 'meta');
		return PGkeyword;
	}
	if (stream.match(/^BEGIN_SOLUTION\s*$/)) {
		state.tokenize = (stream, state) => tokenEV3(stream, state, 'END_SOLUTION', 'meta');
		return PGkeyword;
	}
	if (stream.match(/^BEGIN_PGML\s*$/)) {
		state.tokenize = (stream, state) => tokenPGML(stream, state, 'END_PGML', 'meta');
		return PGkeyword;
	}
	if (stream.match(/^BEGIN_PGML_HINT\s*$/)) {
		state.tokenize = (stream, state) => tokenPGML(stream, state, 'END_PGML_HINT', 'meta');
		return PGkeyword;
	}
	if (stream.match(/^BEGIN_PGML_SOLUTION\s*$/)) {
		state.tokenize = (stream, state) => tokenPGML(stream, state, 'END_PGML_SOLUTION', 'meta');
		return PGkeyword;
	}
	...
	if (ch == '"' || ch == "'") {
		...
		if (prefix(stream, 3) == '<<' + ch) {
			const p = stream.pos;
			stream.eatWhile(/\w/);
			const n = stream.current().substring(1);
	...
	if (typeof ch === 'string' && /[A-Z]/.test(ch)) {
...
export const PG: StreamParser<State> = {
	name: 'PG',
